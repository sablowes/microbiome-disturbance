...
# combine effect sizes from before_after and time series analyses for models fit
# to data standardised across studies
...
aquatic_ba_ES <- alpha_q0_ba_nbinom_shape %>% 
  spread_draws(b_before_afterafter, 
...
alpha_q0_ts_ES <- inner_join(ba_ES %>% 
...
alpha_q0_ts_ES_summary <- alpha_q0_ts_ES %>% 
  ...
  summarise(pop_ba = median(pop_after),
            ...
            pop_slope_median = median(pop_slope),
...
ggplot() +
  ...
  geom_point(data = alpha_q0_ts_ES_summary,
             aes(x = ts_ba, y = ts_slope_median, colour = Environment)) +
  geom_linerange(data = alpha_q0_ts_ES_summary,
  ...
  # overall effects
  geom_point(data = alpha_q0_ts_ES_summary,
             aes(x = pop_ba, y = pop_slope_median), size = 2) +
  geom_linerange(data = alpha_q0_ts_ES_summary,
                 aes(x = pop_ba, ymin = pop_slope_lower, 
                     ymax = pop_slope_upper)) +
  geom_linerange(data = alpha_q0_ts_ES_summary,
                 aes(y = pop_slope_median, xmin = pop_ba_lower, 
                     xmax = pop_ba_upper)) +
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  labs(x = 'Richness response to treatment [log(S)]',
       y = 'Richness change post treatment [log(S) per day]',
       subtitle = expression(paste(alpha, '-scale'))) +
  scale_color_grey(guide = F, start = 0.7, end = 0.7) +
  # scale_color_viridis_d(guide = F, option = 'cividis') +
  theme_minimal() +

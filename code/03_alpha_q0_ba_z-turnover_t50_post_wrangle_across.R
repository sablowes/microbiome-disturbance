...
soil_ba_ES <- alpha_q0_ba_nbinom_shape %>% 
  ...
  select(.chain, .iteration, .draw, Environment, 
         Time_series, before_after, pop_value, ts_value) %>% 
  separate(Time_series, into = c('Study', 'Time_series'), extra = 'merge') %>% 
  # fix 10 names that are screwing up my code
  mutate(Time_series = ifelse(Time_series=='Microcosm_Jurburg_Microcosm.1Heat.Soil',
                              'Jurburg_Microcosm.1Heat.Soil', Time_series),
...
aquatic_t50_ES <- z.resil_time_50_sigma %>% 
  ...
  select(.chain, .iteration, .draw, Environment, 
         Time_series, pop_value, ts_value) %>% 
  separate(Time_series, into = c('Study', 'Time_series'), extra = 'merge') %>% 
  # fix 10 names that are screwing up my code
  mutate(Time_series = ifelse(Time_series=='Microcosm_Jurburg_Microcosm.1Heat.Soil',
                              'Jurburg_Microcosm.1Heat.Soil', Time_series),
...
ggplot() +
  ...
  geom_linerange(data = alpha_ba_resil_t50,
                 aes(y = ts_slope_median, xmin = ts_ba_lower, 
                     xmax = ts_ba_upper,
                     colour = Environment), alpha = 0.25) +
  # overall effects
  geom_point(data = alpha_ba_resil_t50,
             aes(x = pop_ba, y = pop_slope_median, colour = Environment),
             size = 3) +
  geom_linerange(data = alpha_ba_resil_t50,
                 aes(x = pop_ba, ymin = pop_slope_lower, 
                     ymax = pop_slope_upper, colour = Environment),
                 size = 1.25) +
  geom_linerange(data = alpha_ba_resil_t50,
                 aes(y = pop_slope_median, xmin = pop_ba_lower, 
                     xmax = pop_ba_upper, colour = Environment),
                 size = 1.25) +

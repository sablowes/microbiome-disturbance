# dispersion before after results fit to data standardised across studies
...
# these look ok
mcmc_plot(disp_before_after_2_phi2, type = 'trace')

# model reproduces data ok
pp_check(disp_before_after_2_phi2) 
...
# residual check
resid_disp_ba <- residuals(disp_before_after_2_phi2) %>% 
  as_tibble() %>% 
  bind_cols(disp_ba_dat) %>% 
  rename(resid = Estimate) %>% 
  select(-Est.Error, -Q2.5, -Q97.5) 

disp_ba_predicted <- predict(disp_before_after_2_phi2) %>% 
  as_tibble()
...
# recall constrained nature of response
par(mfrow=c(2,3))
...
with(resid_disp_ba, plot(value, predicted, xlab = 'Observed'));abline(c(0,1), lty = 2)

# population (fixed) effects
disp_ba_fixed <- fixef(disp_before_after_2_phi2)

# predictions at the population level (not including uncertainty associated with study-level variation)
disp_ba_fitted <- fitted(disp_before_after_2_phi2, re_formula = NA) %>% 
  as_tibble() %>% 

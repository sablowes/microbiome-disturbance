...
# chains mixed well
mcmc_plot(resil_time_50_phi2_2, type = 'trace')

# model reproduces the data ok
...
resil_dat <- read.delim(paste0(path2wd, 'data/Resilience-within.txt'),
                        sep = " ") %>% 
  as_tibble() %>% 
  filter(Time < 50) %>% 
...
with(resid_resil_50, plot(predicted, value));abline(c(0,1), lty = 2)

...
# model fit to data standardised across studies
...
# chains have mixed and converged
mcmc_plot(disp_time_50_2_phi2, type = 'trace')

# model reproduces data ok
pp_check(disp_time_50_2_phi2)

pp_check(disp_time_50_2_phi2, type = 'scatter_avg_grouped', group = 'Study:Time_series') +
  geom_abline(intercept = 0, slope = 1, lty = 2)

pp_check(disp_time_50_2_phi2, type = 'stat_grouped', group = 'Study:Time_series') +
  theme(strip.text = element_text(size = 6))

# need time on the original scale (not centred), and also the min and max time for each study
dispersion <- read.delim(paste0(path2wd, 'data/dispersions-across.txt'),
                        sep = " ") %>% 
  as_tibble() %>% 
...
disp_50_dat <- disp_time_50_2_phi2$data %>% 
  as_tibble()

# residual check
resid_disp_50 <- residuals(disp_time_50_2_phi2) %>% 
  as_tibble() %>% 
  bind_cols(disp_50_dat) %>% 
  rename(resid = Estimate) %>% 
  select(-Est.Error, -Q2.5, -Q97.5) 

disp_50_predicted <- predict(disp_time_50_2_phi2) %>% 
  as_tibble()
...
# residuals look ok (response is constrained to 0-1)
par(mfrow=c(2,3))
...
with(resid_disp_50, plot(value, predicted));abline(c(0,1), lty = 2)


# population (fixed) effects
disp_50_fixed <- fixef(disp_time_50_2_phi2)

hypothesis(disp_time_50_2_phi2, 'cTime = 0')
hypothesis(disp_time_50_2_phi2, 'cTime + EnvironmentMammal:cTime = 0')
hypothesis(disp_time_50_2_phi2, 'cTime + EnvironmentSoil:cTime = 0')

# predictions at the population level (not including uncertainty associated with study-level variation)
disp_50_fitted <- fitted(disp_time_50_2_phi2, re_formula = NA, scale = 'response') %>% 
  as_tibble() %>% 
  # NB: estimate is predicted value, value is observed
  bind_cols(dispersion_time50)

disp_50_study <- coef(disp_time_50_2_phi2)
...
disp_50_ts <- dispersion_time50 %>% 
  ...
  ungroup() %>% 
  # get fitted values
  tidybayes::add_fitted_draws(model = disp_time_50_2_phi2)

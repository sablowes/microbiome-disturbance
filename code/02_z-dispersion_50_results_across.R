...
# inferences at the time series look not too bad, though 2 are not great
pp_check(zdisp_time_50_sigma2, type = 'scatter_avg_grouped', group = 'Study:Time_series') +
  geom_abline(intercept = 0, slope = 1, lty = 2)

pp_check(zdisp_time_50_sigma2, type = 'stat_grouped', group = 'Study:Time_series') +
  theme(strip.text = element_text(size = 6))


# need time on the original scale (not centred), and also the min and max time for each study
dispersion <- read.delim(paste0(path2wd, 'data/dispersions.zscores-across.txt'),
                         sep = " ") %>% 
  as_tibble() %>% 
